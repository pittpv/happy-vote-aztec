...
- Один голос на Aztec-аккаунт на опрос **или** один голос за календарные сутки UTC, если опрос создан с дневной частотой. Private и open делят этот лимит. Новая Browser session — новый аккаунт, поэтому на опросах с open-eligibility после переподключения можно проголосовать снова. Для fun-опросов (`/p/1`, `/p/2`) это ожидаемо. Personhood/gated (`/p/3`) дополнительно вяжут **один ZKPassport identity**, поэтому лишние session-адреса не дают лишних бюллетеней.

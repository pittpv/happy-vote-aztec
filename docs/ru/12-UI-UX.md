...
На десктопе Connect предпочитает **Azguard** (Aztec 5.2.0, первая кнопка в модалке). **Browser session** — initializerless альтернатива (новый адрес на подключение; без on-chain деплоя аккаунта). **Web Wallet** — Demo Wallet Labs (предпочтительно на iPhone). Prove голоса по-прежнему может быть долгим. После смены разрешений Azguard — отключиться и подключиться снова. Если с этого браузера уже голосовали, UI показывает локальную подсказку (без адреса) и **не** блокирует Vote: повтор с тем же аккаунтом упадёт ончейн.

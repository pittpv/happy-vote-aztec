...
In-app documents (English UI). Effective **15 August 2026**. Contact: **legal@happyvote.xyz**.
...
- **Privacy** — on-chain public state vs private ballots; guest tally API; catalog metadata; hosting logs; first-party cookieless visit aggregates; independent controllers (Aztec Labs RPC, ZKPassport, wallets, explorers).
- **Data Safety** — no passport images; proofs re-verified then discarded from the application path; keys stay in the browser / PXE.
- **Cookies** — first-party functional storage (`localStorage` catalog, short-lived ZKPassport id, participation hint without a wallet address); no first-party analytics cookies. Hosting, fonts, wallets, and ZKPassport may set their own storage.
- **GDPR** — roles, lawful bases, transfers, rights, contact.

Cookie Policy, Privacy Policy, and GDPR state that this subdomain does not run a third-party analytics counter. Visit totals are first-party, cookieless, and aggregated.
